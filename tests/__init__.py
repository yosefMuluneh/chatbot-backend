...
async def test_get_sessions():
    # Create a session
    response = client.post("/chat/new")
    assert response.status_code == 200, "Failed to create session for get_sessions"
...
async def test_send_message():
    # Mock ai_service to avoid external dependencies
    with patch("app.services.ai_service.get_ai_response", return_value="Mock response"):
        new_session_response = client.post("/chat/new")
        assert new_session_response.status_code == 200, "Failed to create session"
